...
def train_model(model, optimizer, dl_train, dl_valid, batch_size, max_epochs, device):
    ...
    for epoch in range(max_epochs):
        ...
        log_step_freq = 1
        step = 0
        for (frame0, frame1), (R, t) in dl_train:
            step += 1
            optimizer.zero_grad()
        ...
        val_loss_sum = 0.0
        val_step = 0
        for (frame0, frame1), (R, t) in dl_valid:
            val_step += 1
            with torch.no_grad():
...
def evaluate_test_set(model, dl_test):
    ...
    test_loss_sum = 0.0
    test_step = 1
    for (frame0, frame1), (R, t) in enumerate(dl_test, 1):
        with torch.no_grad():
            test_loss = total_loss(frame0, frame1, model, test_step)
